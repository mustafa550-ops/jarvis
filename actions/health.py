...
Platform destegi:
  macOS: iPhone Health Export → iCloud Drive (Apple Saglik Export)
  Windows: JSON dosyalari → memory/health/ (manuel kopya veya ucuncu parti arac)
...
if IS_MACOS:
    ...
    _LEGACY_DIRS: list[Path] = [
        MOBILE_DOCS / "com~apple~CloudDocs" / "Auto Export" / "JARVIS",
        MOBILE_DOCS / "com~apple~CloudDocs" / "JARVIS",
    ]
    _LEGACY_FILE: Path | None = MOBILE_DOCS / "com~apple~CloudDocs" / "JARVIS" / "health_data.json"
else:
    BASE = Path(__file__).resolve().parent.parent / "memory" / "health"
    HEALTH_DIR = BASE
    _LEGACY_DIRS: list[Path] = []
    _LEGACY_FILE: Path | None = None
...
def _extract_target_date(query: str) -> date | None:
    ...
    q_text = q
    tokens = q.split()
    multi_word = " ".join(tokens)

    if any(p in multi_word for p in ("onceki gun", "evvelsi gun", "iki gun once")):
        return today - timedelta(days=2)
    if "dun" in tokens or "yesterday" in tokens:
        return today - timedelta(days=1)
    if any(token in tokens for token in ("bugun", "today", "simdi")):
        return today
...
def _resolve_file(target_date: date | None = None) -> Path | None:
    ...
    if _LEGACY_FILE is not None and _LEGACY_FILE.exists():
        return _LEGACY_FILE
    return None
...
def get_health_data(query: str = "all") -> str:
    ...
    if not _resolve_file(target_date):
        ...
        return (
            "Sağlık verisi bulunamadı. "
            "HealthAutoExport-YYYY-AA-GG.json dosyasını "
            "memory/health/ klasörüne kopyala."
        )
